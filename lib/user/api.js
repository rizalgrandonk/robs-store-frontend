...
export const getMejaOptions = async () => {
  const data = await fetch(`${process.env.NEXT_PUBLIC_HOST_URL}/user/table`)
    .then((res) => res.json())
    .then((data) => {
      if (data.code === 400) {
        throw new Error(data.message);
...
export const payOrder = async (formData, token) => {
  const data = await fetch(`${process.env.NEXT_PUBLIC_HOST_URL}/user/pay`, {
    method: "POST",
    headers: {
